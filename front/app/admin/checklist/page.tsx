  ...
  return (
    <div ref={setNodeRef} style={style} className={`bg-white border rounded-xl mb-2 hover:shadow-sm transition-all ${task.completed ? 'opacity-60 bg-gray-50' : ''}`}>
      <div className="p-3 flex items-center gap-3">
        <button {...attributes} {...listeners} className="text-gray-400 hover:text-gray-600 cursor-move"><GripVertical className="w-4 h-4" /></button>
        ...
        <div className="flex-1">
          <input type="text" value={task.title} onChange={(e) => onUpdate(task.id, { title: e.target.value })}
        ...
        <select value={task.priority} onChange={(e) => onUpdate(task.id, { priority: e.target.value })}
                className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded bg-gray-100 ${priorityColors[task.priority as keyof typeof priorityColors]}`}>
          <option value="low">Low</option>
          <option value="medium">Medium</option>
          <option value="high">High</option>
        </select>
        <button onClick={() => onDelete(task.id)} className="text-gray-300 hover:text-red-500 transition-colors"><Trash2 className="w-4 h-4" /></button>
      </div>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="bg-indigo-600 text-white p-4 shadow-md">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2"><ListTodo className="w-5 h-5" /><h1 className="text-xl font-bold">Workflow Checklist</h1></div>
          <button onClick={handleSave} disabled={isSaving} className={`flex items-center gap-2 px-4 py-2 bg-white text-indigo-600 rounded-lg font-semibold hover:bg-indigo-50 transition-all ${isSaving ? 'opacity-70' : ''}`}>
      ...
      <div className="max-w-4xl mx-auto p-6 w-full flex-1">
        <div className="bg-white rounded-2xl shadow-sm border p-6 mb-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={addTask} className="flex items-center gap-1 px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl text-sm font-semibold hover:bg-indigo-100 transition-colors">
