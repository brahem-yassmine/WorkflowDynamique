...
const ActionNode = ({ data }: any) => {
    ...
    return (
        <div className="px-4 py-2 shadow-md rounded-md bg-white border-2 border-blue-500 min-w-[200px]">
            <Handle type="target" position={Position.Left} className="bg-blue-500" style={{ borderRadius: '50%', width: '12px', height: '12px' }} />
            ...
            {data.description && (
                <div className="text-xs text-gray-600 bg-gray-50 p-1 rounded mt-1 border border-gray-100 italic">
                    {data.description}
                </div>
            )}

            <Handle type="source" position={Position.Right} className="bg-blue-500" style={{ borderRadius: '50%', width: '12px', height: '12px' }} />
        </div>
    );
};
