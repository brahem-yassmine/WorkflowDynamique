    ...
    return (
        <div className="flex flex-col space-y-8 animate-in fade-in duration-700">
            ...
            <div className="relative group perspective">
                <div className="absolute -inset-1 bg-gradient-to-r from-indigo-500 to-blue-600 rounded-[44px] blur opacity-20 group-hover:opacity-30 transition duration-1000 group-hover:duration-200"></div>
                <div className="relative flex flex-col md:flex-row justify-between items-center bg-white p-10 md:p-12 rounded-[40px] border border-indigo-50/50 shadow-xl shadow-indigo-500/5 overflow-hidden">
                    ...
                    <div className="absolute top-0 right-0 w-96 h-96 bg-gradient-to-br from-indigo-500/10 to-transparent rounded-full -mr-48 -mt-48 blur-3xl group-hover:scale-110 transition-transform duration-1000"></div>
                    <div className="absolute bottom-0 left-0 w-64 h-64 bg-gradient-to-tr from-blue-500/5 to-transparent rounded-full -ml-32 -mb-32 blur-2xl"></div>
                    
                    <div className="relative z-10 flex items-center gap-8 mb-8 md:mb-0">
                        <div className="w-20 h-20 bg-indigo-600 rounded-3xl flex items-center justify-center text-white shadow-2xl shadow-indigo-200 rotate-3 group-hover:rotate-6 transition-transform duration-500">
                             <Layers size={36} />
                        </div>
                        <div>
                            <div className="flex items-center gap-3">
                                ...
                                <div className="px-3 py-1 bg-indigo-50 rounded-full border border-indigo-100">
                                     <span className="text-[9px] font-black text-indigo-600 uppercase tracking-widest leading-none">Management v2.4</span>
                                </div>
                            </div>
                    ...
                    <div className="flex items-center gap-4 relative z-10">
                        {!selectedModuleId ? (
                            <button 
                                ...
                                className="group/btn relative px-8 py-4 bg-slate-900 text-white rounded-2xl overflow-hidden shadow-2xl transition-all active:scale-95"
                            >
                                <div className="absolute inset-0 bg-gradient-to-r from-indigo-600 to-blue-600 opacity-0 group-hover/btn:opacity-100 transition-opacity" />
                                <div className="relative z-10 flex items-center gap-3">
                                    <Plus size={18} className="group-hover/btn:rotate-90 transition-transform" />
                                    <span className="text-[11px] font-black uppercase tracking-[0.2em]">Deploy New Unit</span>
                                </div>
                            </button>
                        ) : (
                            <button 
                                onClick={() => router.push(`/create-workflow?moduleId=${selectedModuleId}&domainId=${domainId}&isTemplate=true`)}
                                className="group/btn relative px-8 py-4 bg-indigo-600 text-white rounded-2xl overflow-hidden shadow-2xl shadow-indigo-100 transition-all active:scale-95"
                            >
                                ...
                                <div className="relative z-10 flex items-center gap-3">
                                    ...
                                    <span className="text-[11px] font-black uppercase tracking-[0.2em]">Forge Protocol</span>
                                </div>
                            </button>
                        )}
                    </div>
                            ...
                            >
                                {isLoading ? (
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {Array(6).fill(0).map((_, i) => <div key={i} className="h-64 bg-slate-50 rounded-[40px] animate-pulse border border-slate-100" />)}
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        {filteredModules.map((mod, idx) => (
                                            ...
                                            >
                                                ...
                                                <div className="w-full flex items-center gap-2 pt-6 border-t border-slate-50">
                                                    <button 
                                                        onClick={(e) => { e.stopPropagation(); openModuleModal(mod); }} 
                                                        className="flex-1 py-3 bg-white border border-slate-100 hover:border-indigo-100 hover:bg-indigo-50 text-slate-400 hover:text-indigo-600 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all"
                                                    >
                                                        Refine
                                                    </button>
                                                    <button 
                                                        onClick={(e) => { e.stopPropagation(); handleModuleDelete(mod._id); }} 
                                                        className="p-3 bg-white border border-slate-100 hover:border-rose-100 hover:bg-rose-50 text-slate-300 hover:text-rose-500 rounded-2xl transition-all"
                                                    >
                                                        <Trash2 size={16} />
                                                    </button>
                                                </div>
                                    ...
                                    <div className="py-20 flex flex-col items-center justify-center text-center space-y-6">
                                        <div className="w-32 h-32 bg-slate-50 rounded-[40px] flex items-center justify-center border-2 border-dashed border-slate-100 group">
                                            <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
                                                <Plus size={32} className="text-slate-200" />
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            <p className="font-black uppercase tracking-[0.3em] text-slate-400 text-xs">No operational units found</p>
                                            <p className="text-[10px] text-slate-300 font-bold max-w-[200px]">Begin by deploying a functional module to this domain context.</p>
                                        </div>
                                    </div>
                                )}
                            </motion.div>
                                        ...
                                        {templates.map((tpl, idx) => (
                                            ...
                                            >
                                                ...
                                                <div className="flex justify-between items-start mb-8">
                                                    ...
                                                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-500">
                                                        <button 
                                                            ...
                                                            className="p-3 bg-rose-50 text-rose-500 rounded-xl hover:bg-rose-500 hover:text-white transition-colors"
                                                ...
                                                <div className="space-y-2 mb-8">
                                                    <h3 className="font-black text-slate-800 tracking-tight uppercase text-xl group-hover:text-indigo-600 transition-colors">{tpl.name}</h3>
                                                    <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex items-center gap-3 pt-6 border-t border-slate-50">
                                                    <button 
                                                        ...
                                                        className="flex-1 py-3.5 bg-indigo-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100/50 flex items-center justify-center gap-2"
                                                    >
                                                        <Plus size={14} />
                                                        Assign
                                                    </button>
                                                    ...
                                                    <button 
                                                        ...
                                                        className="p-3.5 bg-slate-50 text-slate-400 rounded-2xl hover:bg-indigo-50 hover:text-indigo-600 transition-all"
                                                        title="Edit Template"
                                                    >
                                                        <Edit size={16} />
                                                    </button>
